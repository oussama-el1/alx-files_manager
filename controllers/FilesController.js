...
const writeDataToFile = async (dir, fileName, base64Data) => {
...
class FilesController {
  static async postUpload(req, res) {
    try {
      ...
      try {
        await writeDataToFile(FOLDER_PATH, FILE_NAME, data);
      ...
      const newFile = await dbClient.addFile(name, userId, type, parentId, isPublic, filePath);
  
      try {
        await fileQueue.add({ userId, fileId: newFile.insertedId });
      } catch (err) {
        console.error('Error creating file queue: ', err);
        return res.status(500).json({ error: 'Error creating file' });
      }
